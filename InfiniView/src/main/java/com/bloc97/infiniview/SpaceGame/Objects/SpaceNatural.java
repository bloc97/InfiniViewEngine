...
import com.bloc97.infinisim.Body2D;
import com.bloc97.infiniview.Physics2D.Integrators.FutureContainer;
 ...
 * @author bowen
 */
public class SpaceNatural extends Body2D implements DisplayObject {
    ...
    public SpaceNatural(String name, Vector2 position, Vector2 velocity, double mass, double angPos, double angVel, double radius) {
        super(position, velocity, mass, radius, 0, angPos, angVel);
        this.name = name;
    ...
    public SpaceNatural clone() {
        SpaceNatural newNatural = new SpaceNatural(system, "Clone", position(), velocity(), getMass(), angPosition(), angVelocity(), getRadius());
        return newNatural;
    }
    ...
    public double getSr(Camera camera) {
        double rOut = (Math.log10(getRadius())-6)*3;
        double rIn = getRadius()*camera.getScale();
