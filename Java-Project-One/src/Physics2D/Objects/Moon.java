 ...
 * @author bowen
 */
public class Moon extends PointBody {
    ...
    public Moon(String name, Vector2 position, double mass, double radius) {
        super(position, mass);
        this.radius = radius;
        displayComponent = new PlanetDisplay(name, color, radius);
        update();
    ...
    public Moon(String name, Vector2 position, Vector2 velocity, double mass, double radius) {
    ...
    @Override
    public Moon clone() {
        Moon newTestObject = new Moon("Clone", this.position(), this.velocity(), this.mass(), this.radius);
