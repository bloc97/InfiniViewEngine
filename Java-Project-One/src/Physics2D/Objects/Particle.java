...
public class Particle extends PointBody {
    ...
    public Particle(String name, Vector2 position, double mass, double radius) {
        super(position, mass);
        this.radius = radius;
        displayComponent = new PlanetDisplay(name, color, radius);
        update();
    ...
    public Particle(String name, Vector2 position, Vector2 velocity, double mass, double radius) {
    ...
    public Particle clone() {
        Particle newTestObject = new Particle("Clone", this.position(), this.velocity(), this.mass(), this.radius);
